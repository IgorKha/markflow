...
function rehypeGitHubAlerts() {
  return (tree) => {
    walkBlockquotes(tree, (node, index, parent) => {
      // Find the first <p> child (skip whitespace text nodes)
      ...
      // Strip the [!TYPE] marker (and optional trailing newline) from the text
      firstText.value = firstText.value.slice(match[0].length);

      // Drop the first <p> if it is now completely empty
      ...
      // Replace the blockquote with the alert <div>
        ...
        children: [
          {
            ...
            children: [
              // raw node is serialised as-is by rehype-stringify (allowDangerousHtml)
...
function walkBlockquotes(node, visitor, index, parent) {
  if (node.type === "element" && node.tagName === "blockquote") {
    visitor(node, index, parent);
    // Walk the (potentially replaced) node's children next
    const target = parent.children[index];
    if (target.children) {
      target.children.forEach((child, i) =>
        walkBlockquotes(child, visitor, i, target),
      );
    }
    return;
  }
  if (node.children) {
    node.children.forEach((child, i) =>
      walkBlockquotes(child, visitor, i, node),
    );
  }
}
